...
ButtonsPoll(uint8_t *pui8Delta, uint8_t *pui8RawState)
{
    ...
    // raw value.
    //
    uint8_t b = ROM_GPIOPinRead(BUTTONS_GPIO_BASE, ALL_BUTTONS);
    uint8_t e = ROM_GPIOPinRead(GPIO_PORTE_BASE, GPIO_PIN_1|GPIO_PIN_3|GPIO_PIN_4|GPIO_PIN_5);
    ui32Data = ((b&0x1)<<1) | ((b&0x10)>>2) | ((((e&0x2)<<2) | ((e&0x38)<<1))^0x78);
    if(pui8RawState)
    {
...
ButtonsInit(void)
{
    ...
    ROM_GPIOPinTypeGPIOInput(GPIO_PORTE_BASE, GPIO_PIN_1|GPIO_PIN_3|GPIO_PIN_4|GPIO_PIN_5);
    ...
    uint8_t b = ROM_GPIOPinRead(BUTTONS_GPIO_BASE, ALL_BUTTONS);
    uint8_t e = ROM_GPIOPinRead(GPIO_PORTE_BASE, GPIO_PIN_1|GPIO_PIN_3|GPIO_PIN_4|GPIO_PIN_5);
    g_ui8ButtonStates = ((b&0x1)<<1) | ((b&0x10)>>2) | ((((e&0x2)<<2) | ((e&0x38)<<1))^0x78);
}
